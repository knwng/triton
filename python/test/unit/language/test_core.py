...
def dot_scale_ref(x, scale_x, y, scale_y, type_x, type_y, num_warps):
...
def test_scaled_dot(M, N, K, col_a, col_b, rhs_scale, mxfp_type, normal_type, num_warps, mma, kpack, device):
    ...
    pgm = dot_scale_kernel[(1, )](x, *x.stride(), scale_x, y, *y.stride(), scale_y, z, M, N, K, type_a, type_b,
                                  **kernel_kwargs)
    z_ref = dot_scale_ref(x, scale_x, y, scale_y, type_a, type_b, num_warps)
...
@pytest.mark.parametrize("M, N, K, col_a, col_b, lhs_mxfp_type, rhs_mxfp_type, num_warps, mma, kpack",
                         [(M, N, K, col_a, col_b, lhs_mxfp_type, rhs_mxfp_type, 4, mma, kpack)
                          ...
                          for rhs_mxfp_type in ["e2m1", "e4m3", "e5m2", "e3m2", "e2m3"]
                          for mma in (mma_nonk_sizes if is_hip() else [16])
                          for kpack in ([1, 2] if is_hip() else [1])])
def test_scaled_dot_no_upcast(M, N, K, col_a, col_b, lhs_mxfp_type, rhs_mxfp_type, num_warps, mma, kpack, device):
    if is_cuda() or not is_hip_mi350():
        pytest.skip("Only for AMD MI350")
    ...
    DIV_FACTOR_A = 2 if lhs_mxfp_type == "e2m1" else 1
    DIV_FACTOR_B = 2 if rhs_mxfp_type == "e2m1" else 1
    x = make_arg((M, K // DIV_FACTOR_A), col_major=col_a)
    y = make_arg((K // DIV_FACTOR_B, N), col_major=col_b)
