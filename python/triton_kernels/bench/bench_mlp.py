...
def roofline_mlp(batch_ranges, dim1, dim2, n_expts_tot, n_expts_act, x_dtype, w_dtype, TP=1, EP=1, name="",
    ...
    for batch in batches:
        perfs_mi355x += [bench_mlp(batch, dim1, dim2, n_expts_tot, n_expts_act, x_dtype, w_dtype, TP, EP, name, 'moe_mi355x_logs')]
        perfs_b200 += [bench_mlp(batch, dim1, dim2, n_expts_tot, n_expts_act, x_dtype, w_dtype, TP, EP, name, 'moe_b200_logs')]
    #     if verbose:
    #         print(f"Batch: {batch}; Util: {perfs[-1].util}; TFLOPS: {perfs[-1].tflops}; TBPS: {perfs[-1].tbps}")
    # print("===============================================================")
    # machine limits
    max_tbps = perfs_mi355x[0].max_tbps
    max_tflops = perfs_mi355x[0].max_tflops
    ...
    perf_mi355x = [p.tflops for p in perfs_mi355x]
    perf_b200 = [p.tflops for p in perfs_b200]
    xmin, xmax = min(xs), max(xs)
    dx = 0.05 * (xmax - xmin) if xmax > xmin else 1.0
    ax.set_xlim(xmin - dx, xmax + dx)
    ax.set_ylim(100, max_tflops + 500)
    # plot roofline
    # opints_mi355x = [p.opint for p in perfs_mi355x]
    # knee = bisect_left(opints_mi355x, max_tflops / max_tbps) - 1
    # x_bw, x_comp = xs[:knee], xs[knee:]
    # x_bw = [x_bw[0], x_comp[0]]
    # y_bw = [opints_mi355x[0] * max_tbps, max_tflops]
    # y_comp = [max_tflops] * len(x_comp)
    # ax.plot(x_bw, y_bw, "--", label=f"BW-bound  ({max_tbps:.1f} TB/s)")
    # ax.plot(x_comp, y_comp, "--", label=f"Compute-bound  ({max_tflops:.0f} TFLOP/s)")
    plot_roofline(ax, xs, perfs_mi355x, 'MI355X')
    plot_roofline(ax, xs, perfs_b200, 'B200')
    # plot data
    ax.scatter(xs, perf_mi355x, marker="+", c='b', label='MI355X')
    ax.scatter(xs, perf_b200, marker="*", c='g', label='B200')
    ax.legend(frameon=False, loc="lower right")
    ax.grid(True, which="both", ls=":", lw=0.5)
    fig.tight_layout()
    # fpath = Path(f"logs/{name}/{x_dtype}-{w_dtype}-TP{TP}-EP{EP}/roofline.png")
    fpath = Path(f"roofline_moe_fp8xmxfp4_mi355x_b200.png")
    plt.savefig(fpath)
