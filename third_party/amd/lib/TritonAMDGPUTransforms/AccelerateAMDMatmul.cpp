...
warpsPerTile(ArrayRef<int64_t> shape, int numWarps,
...
warpsPerTile(Operation *dotOp, ArrayRef<int64_t> shape, int numWarps,
  ...
  return warpsPerTile(shape, numWarps, shapePerWarp);
}
...
class ScaledBlockedToMFMANoUpcast final
...
public:
  ScaledBlockedToMFMANoUpcast(MLIRContext *context, int mfmaVersion,
  ...
  LogicalResult matchAndRewrite(triton::DotScaledOp dotOp,
    ...
    if (!supportsTypes(aElemType) || !supportsTypes(bElemType)) {
      llvm::outs()
          << loc
          << " ScaledBlockedToMFMANoUpcast Not supported data type: aElemType: "
          << aElemType << " bElemType: " << bElemType << "\n";
    ...
    if (failed(mfmaInstr)) {
      llvm::outs() << loc
                   << " ScaledBlockedToMFMANoUpcast cannot choose scaled "
                      "mfma intrinsic\n";
    ...
    auto convertInputLayout = [&](TensorValue v, int idx,
                                  ScaleDotElemType type) -> TensorValue {
      auto vType = v.getType();
    ...
    LinearLayout::BasesT retBases = cLL.getBases();
    ...
    // assert(scaleBases[kBlock].empty() && "NYI: CGAs");
    // warpBlockBases[kBlock] = {};
    // auto warpBlock = LinearLayout(std::move(warpBlockBases),
    // standardOutDims); llvm::outs() << " warpBlock A: " << warpBlock << "\n";
    ...
    // auto warpBBlock = LinearLayout(std::move(warpBlockBBases),
    // standardOutDims); llvm::outs() << " warpBlock B: " << warpBBlock << "\n";

    using basisT = std::vector<std::vector<int32_t>>;

    // auto transposeBasisOut2 = [&](const basisT &basis) {
    //   basisT newBasis(basis);
    //   std::reverse(newBasis.begin(), newBasis.end());
    //   // for (const auto &ele : basis) {
    //   //   newBasis.push_back({ele[1], ele[0]});
    //   // }
    //   return newBasis;
    // };
    ...
    auto createLL = [&](int idx, basisT &warpBases) {
      ...
      if (idx == 0) {
        // warps = {{0, 0}, {0, 32}};
        // warps = {{0, 0}, {0, 0}};
        // warps = {{warpBases[0][0], 0}, {warpBases[1][0], 0}};
        // warps = {{0, warpBases[0][1]}, {0, warpBases[1][1]}};
        warps = {warpBases[0], {warpBases[1][1], warpBases[1][0]}};
      } else {
        // warps = {{0, 32}, {0, 0}};
        // warps = {{0, 32}, {0, 64}};
        // warps = {{0, warpBases[0][1]}, {0, warpBases[1][1]}};
    ...
    auto convertScaleLayout = [&](TensorValue scale, int nonKDim, int k,
      ...
      if (!scale) {
        Type scaleType =
            RankedTensorType::get({nonKDim, k / 32}, cast<ShapedType>(i32_ty));
        // 0x7F is 1.0 in E8M0 format
        scale = rewriter.create<SplatOp>(
            dotOp->getLoc(), scaleType,
            rewriter.create<arith::ConstantOp>(
                dotOp->getLoc(), rewriter.getI32IntegerAttr(0x7F)));
      }

      printBases(retBases);
      // LinearLayout lanes = createLL(idx, retBases[kWarp]);
      LinearLayout lanes =
          idx == 0 ? createLL(idx, warpBases) : createLL(idx, warpBBases);
    ...
    auto bScaleShape = bScale.getType().getShape();
    llvm::outs() << "aScaleShape: " << aScaleShape[0] << ", " << aScaleShape[1]
                 << "\n";
    llvm::outs() << "bScaleShape: " << bScaleShape[0] << ", " << bScaleShape[1]
                 << "\n";
    aScale = convertScaleLayout(aScale, aShape[rank - 2], aShape[rank - 1], 0);
    bScale = convertScaleLayout(bScale, bShape[rank - 1], bShape[rank - 2], 1);
    ...
    rewriter.replaceOpWithNewOp<ttg::ConvertLayoutOp>(dotOp, oldRetType,
                                                      newDot);
    llvm::outs() << loc << " ScaledBlockedToMFMANoUpcast succeed\n";
    return success();
  }
};
